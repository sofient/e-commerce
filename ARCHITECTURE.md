# 🏗️ Architecture C.E.R.C.L.E. E-boutique

## 📊 Vue d'ensemble

```
┌─────────────────────────────────────────────────────────────┐
│                    UTILISATEUR                              │
│                  (Navigateur Web)                           │
└────────────────────┬────────────────────────────────────────┘
                     │
        ┌────────────┴───────────┐
        │                        │
        ▼                        ▼
┌──────────────┐         ┌──────────────┐
│   FRONTEND   │         │   SNIPCART   │
│              │         │              │
│  Index.html  │◄────────┤  Panier +    │
│  site.css    │         │  Checkout    │
│  site.js     │         │              │
└──────┬───────┘         └──────┬───────┘
       │                        │
       │ API REST               │ Webhooks
       │                        │
       ▼                        ▼
┌────────────────────────────────────────┐
│           BACKEND API                  │
│         (Node.js/Express)              │
│                                        │
│  ┌──────────────────────────────────┐ │
│  │  Routes & Controllers            │ │
│  │  - Auth (JWT)                    │ │
│  │  - Products (CRUD)               │ │
│  │  - Orders (Gestion)              │ │
│  │  - Snipcart (Webhooks)           │ │
│  └──────────────┬───────────────────┘ │
│                 │                      │
│  ┌──────────────▼───────────────────┐ │
│  │  Middleware                      │ │
│  │  - Security (Helmet, CORS)       │ │
│  │  - Rate Limiting                 │ │
│  │  - JWT Verification              │ │
│  │  - MongoDB Sanitization          │ │
│  └──────────────┬───────────────────┘ │
│                 │                      │
│  ┌──────────────▼───────────────────┐ │
│  │  Services & Utils                │ │
│  │  - Logger (Winston)              │ │
│  │  - Email (Nodemailer)            │ │
│  └──────────────────────────────────┘ │
└────────────────┬───────────────────────┘
                 │
        ┌────────┴────────┐
        │                 │
        ▼                 ▼
┌──────────────┐   ┌──────────────┐
│   MONGODB    │   │    REDIS     │
│              │   │              │
│  - users     │   │  - Sessions  │
│  - products  │   │  - Cache     │
│  - orders    │   │              │
└──────────────┘   └──────────────┘
```

---

## 🗂️ Structure des fichiers

```
cercle-ecommerce/
│
├── 📄 Index.html                    # Page principale (frontend)
├── 📄 about.html                    # Page "Qui sommes-nous"
├── 📄 faq.html                      # Page FAQ
├── 📄 privacy.html                  # Politique de confidentialité
├── 📄 pack-01.html ... pack-10.html # Pages détails produits
│
├── 📁 css/
│   └── site.css                     # Styles Apple-inspired
│
├── 📁 js/
│   └── site.js                      # Scripts frontend
│
├── 📁 backend/
│   │
│   ├── 📄 package.json              # Dépendances Node.js
│   ├── 📄 .env.example              # Template configuration
│   ├── 📄 Dockerfile                # Image Docker backend
│   ├── 📄 .gitignore
│   ├── 📄 README.md                 # Doc API
│   │
│   └── 📁 src/
│       │
│       ├── 📄 server.js             # Point d'entrée Express
│       │
│       ├── 📁 config/
│       │   └── database.js          # Connexion MongoDB
│       │
│       ├── 📁 models/
│       │   ├── User.js              # Schéma utilisateur
│       │   ├── Product.js           # Schéma produit
│       │   └── Order.js             # Schéma commande
│       │
│       ├── 📁 controllers/
│       │   └── authController.js    # Logique authentification
│       │
│       ├── 📁 routes/
│       │   ├── auth.js              # Routes auth
│       │   ├── products.js          # Routes produits
│       │   ├── orders.js            # Routes commandes
│       │   └── snipcart.js          # Routes webhooks
│       │
│       ├── 📁 middleware/
│       │   ├── auth.js              # Vérification JWT
│       │   └── security.js          # Helmet, CORS, etc.
│       │
│       ├── 📁 services/
│       │   └── (futurs services)
│       │
│       ├── 📁 utils/
│       │   └── logger.js            # Winston logger
│       │
│       └── 📁 scripts/
│           └── seed.js              # Alimenter DB
│
├── 📄 docker-compose.yml            # Orchestration Docker
├── 📄 .gitignore                    # Fichiers à ignorer
│
├── 📄 README.md                     # Doc principale
├── 📄 GETTING_STARTED.md            # Guide démarrage
├── 📄 TEST_SCENARIO.md              # Scénarios de test
├── 📄 ARCHITECTURE.md               # Ce fichier
└── 📄 PROJECT_INFO.md               # Infos projet

```

---

## 🔄 Flux de données

### 1. Parcours Achat (Snipcart)

```
┌─────────────┐
│ Utilisateur │
│ clique      │
│ "Ajouter au │
│  panier"    │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Snipcart modal s'ouvre                  │
│ - Récupère infos produit depuis        │
│   data-item-* attributes                │
└──────┬──────────────────────────────────┘
       │
       ▼ (optionnel: validation)
┌─────────────────────────────────────────┐
│ Snipcart appelle:                       │
│ GET /api/v1/snipcart/products/{id}      │
│                                         │
│ Backend valide:                         │
│ - Prix correct                          │
│ - Stock disponible                      │
│ - Retourne infos à jour                 │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Utilisateur remplit formulaire:        │
│ - Email, nom                            │
│ - Adresse livraison                     │
│ - Champs custom (sexe, message)        │
│ - Carte bancaire (Stripe)              │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Snipcart calcule frais livraison       │
│                                         │
│ POST /api/v1/snipcart/webhooks          │
│ Event: shippingrates.fetch              │
│                                         │
│ Backend retourne:                       │
│ - Livraison standard (gratuit si >50€) │
│ - Livraison express (12.90€)           │
│ - Point relais (3.90€)                  │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Paiement Stripe                         │
│ - Tokenization carte                    │
│ - Paiement sécurisé                     │
│ - Success!                              │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Snipcart envoie webhook:               │
│                                         │
│ POST /api/v1/snipcart/webhooks          │
│ Event: order.completed                  │
│                                         │
│ Payload: {                              │
│   invoiceNumber: "ORD-2026-000123"     │
│   items: [...],                         │
│   total: 89.97,                         │
│   email: "client@example.com"           │
│ }                                       │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Backend traite webhook:                │
│                                         │
│ 1. Créer Order en MongoDB               │
│ 2. Décrémenter stock produits           │
│ 3. Calculer donation (15%)              │
│ 4. Logger l'événement                   │
│ 5. (Futur) Envoyer email confirmation   │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Confirmation affichée à l'utilisateur   │
│ - Numéro commande                       │
│ - Email reçu                            │
│ - Tracking URL (si dispo)               │
└─────────────────────────────────────────┘
```

### 2. Parcours Authentification (JWT)

```
┌─────────────┐
│ Utilisateur │
│ clique      │
│ "S'inscrire"│
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ POST /api/v1/auth/register              │
│                                         │
│ Body: {                                 │
│   email: "user@example.com",            │
│   password: "SecurePass123!",           │
│   firstName: "John",                    │
│   lastName: "Doe"                       │
│ }                                       │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ authController.register()               │
│                                         │
│ 1. Vérifier si email existe             │
│ 2. Créer User                           │
│ 3. Hash password (bcrypt, 10 rounds)    │
│ 4. Sauvegarder en MongoDB               │
│ 5. Générer JWT tokens:                  │
│    - accessToken (15min)                │
│    - refreshToken (7 jours)             │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Response 201 Created:                   │
│                                         │
│ {                                       │
│   success: true,                        │
│   data: {                               │
│     user: {...},                        │
│     accessToken: "eyJhbG...",           │
│     refreshToken: "eyJhbG..."           │
│   }                                     │
│ }                                       │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Frontend stocke tokens:                 │
│ - accessToken: localStorage/memory      │
│ - refreshToken: localStorage            │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Requêtes API authentifiées:             │
│                                         │
│ Headers: {                              │
│   Authorization: "Bearer eyJhbG..."     │
│ }                                       │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Middleware auth.verifyToken()           │
│                                         │
│ 1. Extraire token du header             │
│ 2. Vérifier signature JWT               │
│ 3. Vérifier expiration                  │
│ 4. Vérifier user existe et actif        │
│ 5. Attacher req.userId, req.userRole    │
└──────┬──────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────┐
│ Controller exécute action               │
│ - Accès aux données user                │
│ - Vérifications permissions             │
└─────────────────────────────────────────┘
```

---

## 🗄️ Schémas de données

### User Schema

```javascript
{
  _id: ObjectId,
  email: String (unique, required),
  passwordHash: String (hashed, not returned),
  firstName: String,
  lastName: String,
  phone: String,
  sexe: "Homme" | "Femme",

  addresses: [{
    type: "billing" | "shipping",
    street: String,
    city: String,
    postalCode: String,
    country: String,
    isPrimary: Boolean
  }],

  role: "user" | "admin" | "moderator",
  permissions: [String],

  refreshTokens: [{
    token: String,
    expiresAt: Date
  }],

  isVerified: Boolean,
  lastLogin: Date,
  isActive: Boolean,

  createdAt: Date,
  updatedAt: Date
}
```

### Product Schema

```javascript
{
  _id: ObjectId,
  sku: String (unique, "PACK-01"),
  name: String,
  slug: String (unique, "pack-01-enfant-garcon"),
  description: String (long),
  shortDescription: String,

  price: Number,
  discountPrice: Number,
  discountPercentage: Number (auto-calculated),

  stock: Number,
  lowStockThreshold: Number (default: 5),
  isAvailable: Boolean,

  images: [{
    url: String,
    altText: String,
    isPrimary: Boolean
  }],

  category: "enfant-garcon" | "enfant-fille" | ...,
  tags: [String],

  weight: Number (grammes),
  dimensions: { length, width, height },

  averageRating: Number,
  reviewCount: Number,

  donationEnabled: Boolean (default: true),
  donationPercentage: Number (default: 15),

  isActive: Boolean,
  isFeatured: Boolean,
  viewCount: Number,
  soldCount: Number,

  createdAt: Date,
  updatedAt: Date
}
```

### Order Schema

```javascript
{
  _id: ObjectId,
  orderNumber: String (unique, "ORD-2026-000123"),
  userId: ObjectId (ref User, optional),
  guestEmail: String,
  guestName: String,

  items: [{
    productId: ObjectId (ref Product),
    sku: String,
    name: String,
    price: Number,
    quantity: Number,
    subtotal: Number
  }],

  subtotal: Number,
  tax: Number,
  taxRate: Number (default: 20),
  shippingCost: Number,
  discount: Number,
  total: Number,
  donationAmount: Number (15% du subtotal),

  couponCode: String,

  shippingAddress: {
    fullName, street, city,
    postalCode, country, phone
  },

  billingAddress: { ... },

  paymentMethod: "card" | "paypal" | "snipcart",
  paymentProvider: "stripe" | "paypal" | "snipcart",
  paymentStatus: "pending" | "completed" | "failed" | ...,
  transactionId: String,

  orderStatus: "pending" | "confirmed" | "shipped" | ...,
  statusHistory: [{
    status: String,
    date: Date,
    note: String,
    updatedBy: ObjectId
  }],

  shippingCarrier: "colissimo" | "chronopost" | ...,
  trackingNumber: String,
  estimatedDelivery: Date,

  customerNotes: String,
  internalNotes: String,

  createdAt: Date,
  updatedAt: Date
}
```

---

## 🔐 Sécurité - Couches de protection

```
┌────────────────────────────────────────────┐
│  1. HTTPS/TLS (Production)                 │
│  └─ Force HTTPS redirect                   │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  2. CORS                                   │
│  └─ Whitelist origins                      │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  3. Helmet                                 │
│  └─ Security headers (CSP, X-Frame, etc.)  │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  4. Rate Limiting                          │
│  ├─ Global: 100 req/15min                  │
│  └─ Auth: 5 req/15min (anti brute-force)   │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  5. Input Validation                       │
│  └─ Express-validator                      │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  6. MongoDB Sanitization                   │
│  └─ Prévention NoSQL injection             │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  7. JWT Verification                       │
│  ├─ Signature check                        │
│  ├─ Expiration check                       │
│  └─ User active check                      │
└────────────────┬───────────────────────────┘
                 │
┌────────────────▼───────────────────────────┐
│  8. Role-Based Access Control (RBAC)       │
│  └─ requireAdmin, requireRole              │
└────────────────┬───────────────────────────┘
                 │
                 ▼
        ┌────────────────┐
        │   Controller   │
        │   + Service    │
        └────────────────┘
```

---

## 🚀 Déploiement - Architecture Production

```
┌───────────────────────────────────────────────────┐
│               CLOUDFLARE CDN                      │
│  - HTTPS/TLS                                      │
│  - DDoS Protection                                │
│  - Caching statique                               │
└────────────────────┬──────────────────────────────┘
                     │
        ┌────────────┴──────────┐
        │                       │
        ▼                       ▼
┌──────────────┐        ┌──────────────┐
│   VERCEL     │        │   RAILWAY    │
│   (Frontend) │        │   (Backend)  │
│              │        │              │
│  - Index.html│        │  - Node.js   │
│  - Static    │◄───────┤  - Express   │
│    assets    │  API   │  - Docker    │
└──────────────┘  REST  └──────┬───────┘
                               │
                  ┌────────────┴────────────┐
                  │                         │
                  ▼                         ▼
          ┌──────────────┐          ┌──────────────┐
          │ MONGODB      │          │  REDIS       │
          │ ATLAS        │          │  CLOUD       │
          │              │          │              │
          │ - M0 Free    │          │ - Cache      │
          │ - Replica    │          │ - Sessions   │
          │   Set        │          │              │
          └──────────────┘          └──────────────┘
```

---

## 📊 Monitoring & Observabilité

```
┌─────────────────────────────────────────┐
│  APPLICATION                            │
│  └─ Winston Logger                      │
└────────────────┬────────────────────────┘
                 │
        ┌────────┴────────┐
        │                 │
        ▼                 ▼
┌──────────────┐  ┌──────────────┐
│   LOGS       │  │   METRICS    │
│              │  │              │
│ - error.log  │  │ - /health    │
│ - combined   │  │ - Uptime     │
│   .log       │  │ - Response   │
│              │  │   times      │
└──────────────┘  └──────────────┘
```

---

## 🔄 CI/CD Pipeline (Futur)

```
┌─────────────┐
│   GitHub    │
│   Push      │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────┐
│  GitHub Actions                 │
│                                 │
│  1. Run tests (Jest)            │
│  2. Lint code (ESLint)          │
│  3. Build Docker image          │
│  4. Security scan               │
└──────┬──────────────────────────┘
       │
       ▼
┌─────────────────────────────────┐
│  Deploy to Railway              │
│  - Zero downtime                │
│  - Auto rollback si erreur      │
└─────────────────────────────────┘
```

---

**Architecture validée ✅**
**Prêt pour le développement et déploiement**
